...
class Reducer extends Runnable {
  ...
  var reduceinputpath:String =null
  ...
  def this(jobConfiguration: JobConfiguration) {
    ...
    reduceinputpath = LocalConfiguration.dir_ShuffleReceiver + jobconfig.jobID
  }
  ...
  def reducer(outputfile: File, outputFormat: OutputFormat, reduceFunction: Object, intermediate_inputFormat: InputFormat): Unit = {
    ...
    val dir = new File(reduceinputpath)
    val dirfiles = dir.listFiles

    dirfiles.foreach(file => {
      intermediate_inputFormat.getDataReader().read(file)
        .foreach(x => {
